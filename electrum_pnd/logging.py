...
def _shorten_name_of_logrecord(record: logging.LogRecord) -> logging.LogRecord:
    ...
    if record.name.startswith("electrum_pnd."):
...
def _delete_old_logs(path, keep=10):
    files = sorted(list(pathlib.Path(path).glob("electrum_pnd_log_*.log")), reverse=True)
    for f in files[keep:]:
        try:
            os.remove(str(f))
        except OSError as e:
...
def _configure_file_logging(log_directory: pathlib.Path):
    ...
    PID = os.getpid()
    _logfile_path = log_directory / f"electrum_pnd_log_{timestamp}_{PID}.log"
...
electrum_logger = logging.getLogger("electrum_pnd")
...
def get_logger(name: str) -> logging.Logger:
    if name.startswith("electrum_pnd."):
        name = name[13:]
...
def configure_logging(config):
    ...
    if is_android:
        ...
        build_config = autoclass("org.electrum_pnd.electrum_pnd.BuildConfig")
    ...
    _logger.info(f"Electrum-PND version: {ELECTRUM_VERSION} - https://pandacoin.tech - {GIT_REPO_URL}")
